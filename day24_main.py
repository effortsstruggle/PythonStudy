...
2.作用域
    ...
    规则顺序： L –> E –> G –> B。
    局部找不到，便会去局部外的局部找（例如闭包），再找不到就会去全局找，再者去内置中找
    g_count = 0  # 全局作用域
    def outer():
        o_count = 1  # 闭包函数外的函数中
        def inner():    #闭包函数
    ...
    1.模块（module）
    2.类（class）
    3.函数（def、lambda）才会引入新的作用域，
    
    注：其它的代码块（如 if/elif/else/、try/except、for/while等）是不会引入新的作用域的，也就是说这些语句内定义的变量，外部也可以访问
...
4.global 和 nonlocal关键字
    ...
    num = 1
    def fun1():
        global num  # 需要使用 global 关键字声明 num 是全局变量
        print(num) 
        num = 123
        print(num)
    fun1()
